...
#ifndef __iOS7_BACKGROUND_SIDE_VIEW__
...
- (void)viewDidLoad {
    ...
    self.openingSide = kJCMiddleSide;
    self.edgeOffset  = (UIOffset) {
        .horizontal = (__IS_IPHONE__ ? kHorizontaliPhoneOffset : kHorizontaliPadOffset),
        .vertical = 0.0
    };
...
- (CGAffineTransform)enlargeTransformForMenuView {
...
#ifdef __iOS7_BACKGROUND_SIDE_VIEW__
    if (self.openingSide == kJCLeftSide) { // 如果是左边的菜单视图，那么先扩大，再向左平移一定距离
    ...
    else if (self.openingSide == kJCRightSide) {
...
- (void)swipeRight:(UISwipeGestureRecognizer *)sender {
    if (self.openingSide == kJCRightSide) {
...
- (UIViewController *)childViewControllerForStatusBarStyle {
    if (!self.isOpen) {
        ...
        return (self.openingSide == kJCLeftSide) ? self.leftMenuViewController : self.rightMenuViewController;
    }
}
